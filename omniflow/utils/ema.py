...
class EMAModel:
    ...
    @torch.no_grad()
    def step(self, parameters: Iterable[torch.nn.Parameter],one_minus_decay=None):
        if isinstance(parameters, torch.nn.Module):
            deprecation_message = (
        ...
        if one_minus_decay is None:
            one_minus_decay = 1 - decay
        else: # one_minus_decay is force set
        ...
        if self.foreach:
            ...
            for s_param, param in zip(self.shadow_params, parameters):
                ...
                with context_manager():
                    ...
                    #s_param.sub_(one_minus_decay * (s_param - param.to(s_param)))
                    if param.requires_grad:
                        # print(one_minus_decay,decay,"mean",
                        #   (one_minus_decay * (s_param - param.to(s_param)).sum())
                        # )
                        #s_param.sub_(one_minus_decay * (s_param - param.to(s_param)))
                        # s_param - (1 - decay) * (s_param - param.to(s_param))
                        # = decay * s_param + (1 - decay) *  param.to(s_param)
                        s_param.mul_(decay).add_(param.data.to(s_param), alpha=one_minus_decay)
                    else:
                        s_param.copy_(param.data.to(s_param))
